...
class LBXScanWrapper: NSObject,AVCaptureMetadataOutputObjectsDelegate {
    ...
    func captureOutput(_ captureOutput: AVCaptureOutput!, didOutputMetadataObjects metadataObjects: [Any]!, from connection: AVCaptureConnection!)
    {
        ...
        //识别扫码类型
        for current in metadataObjects as [AnyObject]
        {
            if current.isKind(of: AVMetadataMachineReadableCodeObject.self)
            {
                ...
                print("code type:%@",codeType ?? "codeType为空")
                //码内容
                let codeContent = code.stringValue
                print("code string:%@",codeContent ?? "codeContent为空")
    ...
    func connectionWithMediaType(_ mediaType:String,connections:[AnyObject]) -> AVCaptureConnection?
    {
        for connection:AnyObject in connections
        {
            let connectionTmp:AVCaptureConnection = connection as! AVCaptureConnection
            for port in connectionTmp.inputPorts as [AnyObject]
            {
                if port.isKind(of: AVCaptureInputPort.self)
                {
    ...
    //MARK:切换识别区域
    func changeScanRect(_ cropRect:CGRect)
    {
        //待测试，不知道是否有效
        stop()
        output.rectOfInterest = cropRect
    ...
    //MARK: 切换识别码的类型
    func changeScanType(_ objType:[String])
    {
        //待测试中途修改是否有效
        output.metadataObjectTypes = objType
    }
...
//        return Float(UIDevice.currentDevice().systemVersion)  >= 8.0 ? true:false
    ...
    static func createCode( _ codeType:String, codeString:String, size:CGSize,qrColor:UIColor,bkColor:UIColor )->UIImage?
    {
        if #available(iOS 8.0, *){
    ...
    static func createCode128(  _ codeString:String, size:CGSize,qrColor:UIColor,bkColor:UIColor )->UIImage?
    {
        let stringData = codeString.data(using: String.Encoding.utf8)
    ...
    //根据二维码的区域截取二维码区域图像
    static func getConcreteCodeImage(_ srcCodeImage:UIImage,rect:CGRect)->UIImage?
    {
        if rect.isEmpty
        {
            return nil
    ...
    //获取二维码的图像区域
    static func getConcreteCodeRectFromImage(_ srcCodeImage:UIImage,codeResult:LBXScanResult)->CGRect
    {
    ...
    @return 加Logo的图像
    */
    static func addImageLogo(_ srcImg:UIImage,logoImg:UIImage,logoSize:CGSize )->UIImage
    {
